...
The Promega initially shipped with a choice of two extruder heads: the K'Tana and the Compound. The Compound head accepts two filaments as input and prints through one nozzle by mixing the two filaments. The K'Tana has two separate filament inputs and two seperate nozzles; each input on the K'Tana is routed to a separate nozzle. As such, there is no mixing of filaments in the K'Tana extruder as there is in the Compound.

In order to switch between the two tools, you will have to perform a series of mechanical and software changes.

## Preparation

Be sure to turn off and unplug the printer. 

Unload any filament currently in the nozzle. Use [this guide](https://m3d.gitbook.io/promega-docs/getting-started/loading-and-unloading-filament) if you need help. Wait for the hot end\(s\) of the printer to cool before continuing with this guide.
...
1. The cold section fan is attached to the extruder head with two screws, with a M4 nut on each one as a spacer. Take the T10 Torx screwdriver and remove the cold section fan from the extruder carriage. Collect the two screws and M4 nuts in a container and place them aside. Move the fan over to the back of the extruder carriage assembly so it is out of the way.

   ![PBezwNA9DMD3QPkF-RemoveFan.jpg](../.gitbook/assets/pbezwna9dmd3qpkf-removefan.jpg)

2. Use the T30 Torx screwdriver to remove the three screws \(circled in red\) holding the extruder block to the coreXY gantry. Place these screws in the container.
...
4. Next, reinstall the fan onto the extruder head you just mounted. Remember to put the M4 spacers in place as well. Place the wires of the fan below the second screw as shown in the picture below.
...
1. Download the SD card image from our GitHub [Repository](https://github.com/PrintM3D/Promega). Select either the Compound or K'Tana folder depending on which extruder you are mounting. The SD card should have 4 folders and one text file:
2. _sys/_
3. _gcodes/_
4. _macros/_
5. _www/_
...
1. Plug your microSD card into your computer with the microSD card reader. 
...
6. Once you have finished configuring your network, eject the card. Be sure the Promega is powered down, and has no powered USB connection. Insert it into the Duet's microSD card slot with the brass-colored contacts side facing INTO the Promega.
